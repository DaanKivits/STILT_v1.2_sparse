...
! Convert Navy COAMPS output files to ARL format compatible for input
...
! used in subsequent subroutines for both input and output data grids
      ...
      PARAMETER (MXP=200, MYP=200, MZP=50)
...
!     define equivalent COAMPS variables
      DATA XCHAR0/ 'tepr','prcp','strs','strs','hfls','topo'/
      DATA XCHAR1/ 'pppp','uuuu','vvvv','wwww','pott','wvap'/
...
!     grid information is decoded from the COAMPS data file
      CALL HEADER(DATAD,BASEF,NGRID,MZP,MXYP,NSIGMA,MPTYPE,                    &
         TNGLAT,ALIGN,NXP,NYP,DELX,XLAT1,XLON1,SIGMA,DSIGMA)

      CALL WINDEX(NGRID,NSIGMA,MPTYPE,TNGLAT,ALIGN,NXP,NYP,                    &
         DELX,XLAT1,XLON1,SIGMA,MVAR,NVAR,VCHAR0,VCHAR1,FNAME)
...
! Information checked if supported under ARL format conventions
      ...
      LOGICAL   FTEST
      REAL      HDATA(500), SIGMA(MZP), DSIGMA(MZP)
      CHARACTER FNAME*80, DATAD*80, BASEF*10

!     construct file name     hhhmmss    level-1  level-2
      FNAME='dataa#'//BASEF//'0000000'//'00000'//'00000'//'sfl'
!     header file only written for grid #1
      WRITE(FNAME(6:6),'(A1)')'1'
...
!        number of grid points in the x and y directions
         NXP=INT(HDATA( (NGRID+2)*10+0 ))
         NYP=INT(HDATA( (NGRID+2)*10+1 ))
...
!        grid spacing (m) in the x and y directions
         DELX=HDATA(9) /3**(NGRID-1)
         DELY=HDATA(10)/3**(NGRID-1)
...
!        latitude and longitude of 1,1 nest grid point
         XLAT1=HDATA( (NGRID+2)*10+6 )
         XLON1=HDATA( (NGRID+2)*10+7 )

!        enter dsigma level heights into array
         DO K=1,NSIGMA
            DSIGMA(K)=HDATA(100+K)
         END DO
...
!     file name construct and testing
      CHARACTER FNAME*36, DATAD*80, BASEF*10, BTIME*7
      LOGICAL FTEST
...
!     coamps equivalent character IDs for surface and upper level
      CHARACTER*4 XCHAR0(MVAR), XCHAR1(MVAR)
...
!        special processing for variables with no equivalent
         IF(XCHAR0(N).EQ.'topo')THEN
!           special constant fields base time = 0
            BTIME='0000000'
         ELSEIF(XCHAR0(N).EQ.'prcp')THEN
...
!          construct base time (hhhmmss) for file name
           BTIME='###0000'
           WRITE(BTIME(1:3),'(I3.3)')KHOUR
           IF(XCHAR0(N).EQ.'strs')THEN
...
!        construct file name
         FNAME=XCHAR0(N)//'a#'//BASEF//BTIME//'00000'//'00000'//'sfl'
...
!        test for existence of data file before proceeding to decoder
         ...
         IF(FTEST)THEN
            ...
            WRITE(*,*)'Started processing: ',FNAME
            READ(10)((RVAR(I,J,1),I=1,NXP),J=1,NYP)
            CLOSE (10)
...
!        col:    1->4    56   7->16  17->23  24->28   29->33  34->36
         FNAME=XCHAR1(N)//'a#'//BASEF//BTIME//'00000'//'00000'//'sgl'

!        update name with level information
         WRITE(FNAME(24:28),'(I5.5)')INT(SIGMA(1))
         WRITE(FNAME(29:33),'(I5.5)')INT(SIGMA(NSIGMA))
         IF(XCHAR1(N).EQ.'wwww')THEN
            WRITE(FNAME(24:28),'(I5.5)')                                       &
                  INT(SIGMA(1)+DSIGMA(1)/2.0)
            WRITE(FNAME(29:33),'(I5.5)')                                       &
