...
!     converts an rsm model grib file to arl packed format
...
!
! USAGE:  RSMS2ARL [GRIB_FILE_NAME] [GRIB_FILE_NAME2] [...]
!   INPUT ARGUMENT LIST:
...
!     CALL W3TAGB('RSMS2ARL ',1998,0238,0070,'R/E/AR ')

!=>check for command line arguments
      NARG=IARGC()

      if(narg.eq.0)then
         write(*,*)'Usage: rsms2arl [file_1] [file_2] [...]'
         stop
      end if
...
!     set output structure by defining GRIB variables (sfc + level 1)
      DATA VGRIB0/  1, 61,124,125,122,121 /
      DATA VGRIB1/ 33, 34, 39, 11, 51,  0 /

!     set output structure in ARL character format
      DATA VCHAR0/ 'PRSS','TPP6','UMOF','VMOF','SHTF','LHTF'/
      DATA VCHAR1/ 'UWND','VWND','WWND','TEMP','SPHU','    '/

!     set requested sigma levels for input and output
      DATA SIGL/10000,9950,9821,9644,9425,9159,8838,8458,8014,7508,            &
                 6943,6329,5681,5017,4357/

!     units conversion factors before converting to ARL format
      DATA CNVRT0/ 0.01, 0.001,  1.0,  1.0,  1.0,  1.0 /
      DATA CNVRT1/  1.0,   1.0, 0.01,  1.0,  1.0,  1.0 /
...
!     check if 2d variable present in selection table
      ...
      do kv=1,nvar(kl)
         ...
         vchar=vchar0(kv)
         cnvrt=cnvrt0(kv)
         if(kvarb.eq.vgrib.and.ktype.eq.1)go to 300
      end do
