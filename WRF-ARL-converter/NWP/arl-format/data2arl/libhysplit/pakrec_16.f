...
SUBROUTINE PAKREC_16(LUNIT,RVAR,CVAR,NX,NY,NXY,KVAR,IY,IM,ID,IH,MN,IC,LL,KINI,use_16)
...
!==>check if requested unit number properly opened
  ...
  END DO
  IF(KG.EQ.0)THEN
     WRITE(*,*)'*ERROR* pakrec_16: Requesting uninitialized unit'
...
!==>test grid dimensions for consistency with initialization
  ...
  if (use_16) nxyg=2*nxyg
  IF(GV(KG)%NXG.NE.NX.OR.GV(KG)%NYG.NE.NY.OR.NXYG.NE.NXY)THEN
     WRITE(*,*)'*ERROR* pakrec_16: file dimensions do not match'
...
!==>check current if current record consistent with first
     ...
     IF(IY.NE.GV(KG)%IY0.OR.IM.NE.GV(KG)%IM0.OR.                           &
        ...
        WRITE(*,*)'*ERROR* pakrec_16 - at index: ',GV(KG)%MREC
...
!==>check vertical index
  ...
  IF(LL.LT.1.OR.LL.GT.GV(KG)%NLVL)THEN
     WRITE(*,*)'*ERROR* pakrec_16  : Level indicator out of range'
...
!==>compute the record offset based upon variable match
  ...
  IF(NV.EQ.0)THEN
     WRITE(*,*)'*ERROR* pakrec_16: Variable not in CFG file'
...
! convert real to packed character
  CALL PAKOUT_16(RVAR,CVAR,NX,NY,NXY,PREC,NEXP,VAR1,KSUM,use_16)
  ...
  IF(GV(KG)%XGPT)THEN
     if (k50 .eq. 50) then
        ...
        WRITE(LABEL(1:k50),'(5I2,a1,i3,A2,A4,I4,2E14.7)')            &
             IY,IM,ID,IH,ICW,'X',IL,GV(KG)%IGC,KVAR,NEXP,PREC,VAR1
     end if
  ELSE
     if (k50 .eq. 50) then
        ...
        WRITE(LABEL(1:k50),'(5I2,a1,i3,i2,A4,I4,2E14.7)')              &
             IY,IM,ID,IH,ICW,'X',IL,GV(KG)%IG,KVAR,NEXP,PREC,VAR1
     end if
  END IF
...
! compute record based upon variable offset
  ...
  IF(JREC.LE.1)THEN
     WRITE(*,*)'*ERROR* pakrec_16: output record <=1'
     WRITE(*,*)'  Index record: ',GV(KG)%MREC
     WRITE(*,*)'  Level offset: ',GV(KG)%NREC(LL)
     WRITE(*,*)'  Varbl offset: ',NV
     STOP
  ELSE
     WRITE(GV(KG)%KUNIT,REC=JREC)LABEL(1:k50),CVAR
  END IF

END SUBROUTINE pakrec_16
