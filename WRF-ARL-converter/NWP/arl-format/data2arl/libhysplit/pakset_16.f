...
!-------------------------------------------------------------------------------
  ...
  character (len=30) :: deffmt='(20X,F6.0,I3,99(1X,A4))'
...
!==>compute extended header length
  ...
  DO L=1,NLVL
     ...
     END DO
  END DO
  if (GV(NG)%LENH .ge. 10000) GV(NG)%LENH=GV(NG)%LENH+2

!==>check for multiple extended header records

  GV(NG)%LREC=(GV(NG)%NXG*GV(NG)%NYG)
  if (use_16) GV(NG)%LREC = 2*GV(NG)%LREC
! check limits
  IF((GV(NG)%LENH).GT.MLEN.OR.(GV(NG)%LREC).LT.108)THEN
     WRITE(*,*)'*ERROR* pakset_16: Extended header exceeds limits...'
...
! check validity of index record
  ...
  IF(MOD(KREC-1,NTOT).NE.0)THEN
     WRITE(*,*)'*ERROR* pakset_16: position record not even multiple'
     WRITE(*,*)'  Record numb in argument:',KREC
     WRITE(*,*)'  Records per time period:',NTOT
     STOP
  ELSE
     WRITE(*,*)' NOTICE pakset_16:'
