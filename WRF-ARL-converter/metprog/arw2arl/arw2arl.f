...
!---------------------------------------------------------------------------
  ...
  LOGICAL       :: difr, difw
  INTEGER       :: k, l, n, nt1, nt2, ntph, kcfg
  INTEGER       :: n3d, ndim, varid, narg, iargc, clen, mzp, maxl
  INTEGER       :: ntp, krec, nxp, nyp, nzp, nxy, kret, kinit
...
! subroutine interfaces
  ...
  SUBROUTINE getdim (diag,ncid,ntp,nxp,nyp,nzp,clen)
  IMPLICIT NONE
  ...
  SUBROUTINE setmap (diag,ncid,nxp,nyp)
  IMPLICIT NONE
  LOGICAL,       INTENT(IN)  :: diag
  INTEGER,       INTENT(IN)  :: ncid
...
! open the NetCDF input file and obtain basic information to design output
  ...
  CALL getdim (diag,ncid,ntp,nxp,nyp,nzp,clen)
...
! create the ARL packing information file 
  ...
  IF(.NOT.ftest)THEN
     CALL cfgrec (kunit,arlcfg_file,nxp,nyp,mzp,level(:,1),num2dv,arl2dv,num3dv,arl3dv)
  ELSE
...
!             write the difference to the output file 
     ...
     DO n=1,num3dv
        ...
        DO k=1,MZP   
           ...
           ELSE
!             currently the generic variable only used for TKE due to differences
!             in variable name according the PBL scheme or WRF version
              base=cnv3dv(n)*var3d(:,:,k+1)
           END IF
