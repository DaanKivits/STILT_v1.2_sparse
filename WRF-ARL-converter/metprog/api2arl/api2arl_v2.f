...
!               08 Feb 2013 (AFS) - numatm->numsfc in unknown
!-------------------------------------------------------------
...
! library: ./hysplit4/library/libhysplit.a
  ...
  SUBROUTINE MAKNDX (FILE_NAME,MODEL,NXP,NYP,NZP,CLAT,CLON,DLAT,DLON,   &
                     ...
                     SFCVAR,ATMVAR,ATMARL,SFCARL,INVERT,LTYPE)
  IMPLICIT NONE
  ...
  CHARACTER(8), INTENT(IN)   :: ltype         ! level type          
...
!          level within vertical domain
           IF(trim(ltype).EQ.'pl'.AND.levhgt.GE.top.OR.  &
              trim(ltype).EQ.'ml'.AND.levhgt.LE.top      )THEN
...
!    IF(invert.AND.trim(project).EQ.'lambert')THEN
!       N-S index increases with decreasing latitude
!       IF(param.EQ.'VWND') units=-units
!    END IF
...
SUBROUTINE makapi(apicfg_name)
  ...
  write(30,'(a)')' numatm = 10,'
  write(30,'(a)')' atmgrb = '//a//'gh'//d//'pres'//d// 't'//d//'pt'//d//'u'//d//'v'//d//'w'//d//'z' &
                             //d// 'r'//d//'qv'//a//c
  write(30,'(a)')' atmcat =     3 ,   3 ,    0 ,   0 ,   2 ,    2 ,    2 ,   2 ,   1 ,   1 ,'  
  write(30,'(a)')' atmnum =     5 ,   0 ,    0 ,   2 ,   2 ,    3 ,    8 ,   9 ,   1 ,   2 ,' 
  write(30,'(a)')' atmcnv =   1.0 ,0.01 ,  1.0 , 1.0 , 1.0 ,  1.0 , 0.01 , 1.0 , 1.0 , 1.0 ,'
  write(30,'(a)')' atmarl = '//a//'HGTS'//d//'PRES'//d//'TEMP'//d//'THET'//d//'UWND'  &
                             //d//'VWND'//d//'WWND'//d//'DZDT'//d//'RELH'//d//'SPHU'//a//c
  ...
  write(30,'(a)')' sfccat =   3,    3,    1,   6,  19,   2,  17,   4,   0,   0,   0,   1,   2,   2,' 
  write(30,'(a)')' sfcnum =   5,    0,    8,   1,   3,  17,  18,   0,  11,  10,   0,   1,   2,   3,' 
