...
!          test against GRIB code number
        ...
        IF(kv.NE.-1)THEN
           ...
           msglev(i)=0
           msgvar(i)=kv
           IF(sfcarl(kv).EQ.'TPPA')THEN
!             both convective and stratus precip in combined field
!             set flag for only one of the two
              krain=krain+1
...
!    special processing for precipitation
     IF(PARAM.EQ.'TPPA')THEN 
