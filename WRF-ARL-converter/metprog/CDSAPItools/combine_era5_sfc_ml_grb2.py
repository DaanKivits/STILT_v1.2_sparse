...
endtime = dtm.datetime(2021,1,31)
for date in rrule.rrule(rrule.MONTHLY, dtstart=starttime, until=endtime):
    ...
    # This is the outfile to process: pressure levels!
    ...
    # Then merge into this new intermediate nc file:
    intermediatefile = f'/projects/0/ctdas/PARIS/DATA/meteo/STILT/combined_20lvls/ERA5intermediate-{date:%Y%m}.grb'
    ...
    # Then convert into this GRIB2 file:
    newfile = f'/projects/0/ctdas/PARIS/DATA/meteo/STILT/combined_20lvls/ERA5-{date:%Y%m}.grb2'
    
    # Convert to GRB2
    command = f'cdo -v -f grb2 -copy {intermediatefile} {newfile}'
    p = subprocess.Popen(command, shell=True)
    p.communicate()
    ...
    newfile = f'/projects/0/ctdas/PARIS/DATA/meteo/STILT/combined_20lvls/ERA5-{date:%Y%m}.grb'
...
"""
    ...
    for i in range(ndays_month):
        ...
        dailyfile = f'/projects/0/ctdas/PARIS/DATA/meteo/STILT/combined_20lvls/ERA5-{date:%Y%m}{d:02}.grb2'
        command = f'cdo seltimestep,{starthour}/{endhour} {newfile} {dailyfile}'
        p = subprocess.Popen(command, shell=True)
        p.communicate()

    # Remove the intermediate files
    os.remove([newfile, intermediatefile])
