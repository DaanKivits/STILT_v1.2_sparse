Trajec<-function(yr=02,mon=8,day=1,hr=6,lat=42.536,lon=-72.172,agl=30,nhrs=-48,
           delt=0.0,numpar=100,ndump=0,random=T,outdt=0.0,veght=0.5,metlib="/deas/group/stilt/Metdata/",
           ...
           sigzierr=NULL,TLzierr=NULL,horcorzierr=NULL,zsg.name=NULL,create.X0=FALSE)
{
   ...
   #'doublefiles' should concatenated met files be used? Allows starting times between files concatenation with "cat file1 file2 > file12"
   ...
   #'ziscale'     is a vector with which to scale the modelled mixed-layer height; each element specifies scaling factor for each model simulation hour (ziscale can be of length that is smaller than abs(nhrs))
   ...
   #'varsout'     specifies output variables from STILT; can be any subset of c("time","sigmaw","TL","lon","lat","agl","grdht","index","cldidx","temp","temp0","sampt","foot","shtf","lhtf","tcld","dmass","dens","rhf","sphu","solw","lcld","zloc","swrad","wbar","zi","totrain","convrain","zconv","pres")
   ...
   #'outname'     specifies name of the object for output; if not specified, uses default name based on time and position using pos2id() (e.g. "2002x08x16x06x42.54Nx072.17Wx00030")
...
#  unix.shell(paste("sync",sep=""),shell="/bin/csh")   #sync after to make sure files are writen before read
   ...
   if (debugTF) 
   {
      ...
      file.copy(from=paste(rundir,save.file,sep=""), to=paste(rundir,save.file,".",loop.counter,".",outname,sep=""),overwrite = TRUE)
   }
