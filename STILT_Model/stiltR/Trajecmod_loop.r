...
Trajecmod <- function(partarg=NULL, totpartarg=NULL, nodeoffset=NULL) 
{
   ...
   {
      file.copy("setStiltparam_loop.r",paste(runs.done.dir, "setStiltparam", savename, ".r", sep=""), overwrite=T)
      cat("Saving copy of setStiltparam_loop.r in ", paste(runs.done.dir, "setStiltparam", savename, ".r", sep=""), "\n",sep="")
   } 
   else 
   {
   ...
   #     In this form, Trajecfoot only updates 'foot'. Internally it reads the wrfinput-file
         foot <- Trajecfoot(identname, pathname=path, foottimes=foottimes, zlim=c(zbot, ztop),
                            ...
                            lon.ll=lon.ll, lat.ll=lat.ll, lon.res=lon.res, lat.res=lat.res,landcov="IGBP",wrfinput=NULL) #"/projects/0/ctdas/WRF-input/wrfout/run04/wrfout_d02_2016-01-01_00:00:00")
...
#            Write Output to newly created nc file
             ...
             nc_foot        <- ncvar_def( "foot","ppm/(ug/m2/s)", list(dimx,dimy,dimt), mv, prec="float")
             #nc_foot2       <- ncvar_def( "foot","ppm/(ug/m2/s)", list(dimx,dimy,dimt), mv, prec='double')
             nc             <- nc_create(outfilename, list(nc_foot))
             ncvar_put(nc, nc_foot, aperm(foot, c(2,1,3)), start=c(1, 1, 1),count=c(-1, -1, -1))
             #ncvar_put(nc, nc_foot, foot, start=c(1, 1, 1),count=c(-1, -1, -1)) 
             nc_close(nc)
       }
