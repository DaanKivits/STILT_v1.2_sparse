...
Trajecmod <- function(partarg=NULL, totpartarg=NULL, nodeoffset=NULL) 
{
   ...
   #     In this form, Trajecfoot only updates 'foot'. Internally it reads the wrfinput-file
         foot <- Trajecfoot(identname, pathname=path, foottimes=foottimes, zlim=c(zbot, ztop),
                            ...
                            lon.ll=lon.ll, lat.ll=lat.ll, lon.res=lon.res, lat.res=lat.res,landcov="IGBP",wrfinput=NULL, sparse=TRUE) #"/projects/0/ctdas/WRF-input/wrfout/run04/wrfout_d02_2016-01-01_00:00:00")

         # Check length of footprint
         #print(paste("length(foot)=",length(foot),sep=""))
         #print(paste("length(foottimes)=",length(foottimes),sep=""))
         #print(foot)
...
#            Write Output to newly created nc file
             ...
             dimy         <- ncdim_def( "Latitude" ,"degrees N", latx[1,])
             dimnonzero <- ncdim_def( "Nonzero_len", "Amount of non-zero values", 1:nonzerolen, create_dimvar=TRUE, unlim=TRUE)
             nc_foot        <- ncvar_def( "foot","ppm/(ug/m2/s)", dimnonzero, mv, prec="float", compression = 6)
             nc_times      <- ncvar_def( "timestamp","Time in UTC", dimnonzero, mv, compression = 6)
             nc_index_x       <- ncvar_def( "index_x","[-]", dimnonzero, mv, prec="short", compression = 6)
             nc_index_y      <- ncvar_def( "index_y","[-]", dimnonzero, mv, prec="short", compression = 6)
             nc             <- nc_create(outfilename, list(nc_foot))
             ncvar_put(nc = nc, varid = nc_foot, vals = values, start=c(1), count=c(-1))
             ncvar_put(nc = nc, varid = nc_times, vals = times, start=c(1), count=c(-1))
             ncvar_put(nc = nc, varid = nc_index_x, vals = index_x, start=c(1), count=c(-1))
