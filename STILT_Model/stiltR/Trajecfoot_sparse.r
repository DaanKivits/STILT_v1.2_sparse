...
# 'wrfinput'        for projection of footprints on WRF grid: absolute path to WRF input file (incl. filename)
# 'sparse'          controls whether footprints are stored in a sparse or dense format
# 
# ---------------------------------------------------------------------------------------------------    
# $Id: Trajecfoot.r,v 1.17 2009/02/11 13:03:13 gerbig Exp $
# EDIT (06072023, Daan K) Sparse footprints functionality added
# ---------------------------------------------------------------------------------------------------
...
if(is.null(part))
   ...
   if(length(unix.shell(paste("if (-e ",pathname,".RData",ident,") echo 'found it!'; endif",sep=""),shell="/bin/csh"))>0)
      ...
      {  #if not there, break out of function, return NA's
...
if(!is.null(fluxweighting))
   ...
   if(fluxweighting=="CO2"|fluxweighting=="CO")
      {
       if(is.null(timelabel))
          { #if timestamp not passed on, then take from 'ident'
...
if(is.null(wrfinput))
  ...
  gity<-floor(1/lat.res*(part[,"lat"]-lat.ll)+1)
   ...
   { #use wrfinput to get projection info. Note that the output grid is set in setStiltparam.r. You have to manually input the grid information there.
#   print(paste("Trajectfoot 106: numpix.(x,y)=(",numpix.x,",",numpix.y,"); (lon,lat)ll=(",lon.ll,",",lat.ll,"); (lon,lat)res=(",lon.res,",",lat.res,")"))

#   On CapeGrim:
#   require(RNetCDF)
#   nc             <- open.nc(wrfinput) #
#   latx           <- var.get.nc(nc,"XLAT_V")                     #,start=c(1,1,1),count=c(NA,NA,1)) #diff columns 1-ny = N-S, diff rows 1-nx = W-E
#   longx          <- var.get.nc(nc,"XLONG_U")                    #,start=c(1,1,1),count=c(NA,NA,1)) #using staggered grid, to get boundaries between cells
...
#   On Cartesius
    require(ncdf4)
    nc             <- nc_open(wrfinput)
    latx           <- ncvar_get(nc,"XLAT_V", start=c(1,1,1),count=c(-1,-1,1)) #   in R: (nx,ny,nt) = (320, 229, 24) (dimensions have a different order than in Python)
    longx          <- ncvar_get(nc,"XLONG_U",start=c(1,1,1),count=c(-1,-1,1))
    moadstandlat_1 <- ncatt_get(nc,0,"TRUELAT1")
    moadstandlat_2 <- ncatt_get(nc,0,"TRUELAT2")
    moadstandlon   <- ncatt_get(nc,0,"STAND_LON")    
    moadknownlat   <- ncatt_get(nc,0,"MOAD_CEN_LAT")
    moadknownlon   <- ncatt_get(nc,0,"CEN_LON")
    dx             <- ncatt_get(nc,0,"DX")
    dy             <- ncatt_get(nc,0,"DY")
    nc_close(nc)

    # Next 7 lines only on Cartesius:
    moadstandlat_1 <- moadstandlat_1$value
    moadstandlat_2 <- moadstandlat_2$value
    moadstandlon   <- moadstandlon$value
    moadknownlat   <- moadknownlat$value
    moadknownlon   <- moadknownlon$value
    dx             <- dx$value
    dy             <- dy$value

#   xy: convert particle locations (lon,lat) = (part[,"lon"] and part[,"lat"]) to (x,y) coordinates.
#   part: contains 4887 rows and 18 columns: time, index, lat, lon, agl, grdht, foot, temp0, swrad, zi, dens, dmass, totrain, convrain, zconv, sigmaw, TL, pres

#   Make projection using proj4 project.nice: convert (lon,lat) to (x,y)
#   function(x.in,y.in,proj="+proj=lcc +lat_1=30 +lat_2=60 +lat_0=50 +lon_0=10",inv=FALSE,matTF=TRUE,matinTF=FALSE)
...
#   print(xy.corner): -712422.3, -568827.4
    xy             <- project.nice(x.in=part[,"lon"], y.in=part[,"lat"],proj=projout, inv=FALSE, matTF=FALSE, matinTF=FALSE)      # using staggered grids
    gitx           <- floor( (xy[[1]]-xy.corner[[1]])/dx+1)                                                                       # index ix in lon-row (1-based)
    gity           <- floor( (xy[[2]]-xy.corner[[2]])/dy+1)                                                                       # index iy in lat-row (1-based)
    #set/reset grid definition
    numpix.y       <- dim(longx)[2]                                                                                               # staggered, so use lon (or drop last column of lat)
    numpix.x       <- dim(latx) [1]
#   print(paste("Trajectfoot 148: numpix.(x,y)=(",numpix.x,",",numpix.y,"); (lon,lat)ll=(",lon.ll,",",lat.ll,"); (lon,lat)res=(",lon.res,",",lat.res,")"))

   }
part               <- cbind(part,gitx,gity)             # add gitx and gity as columns to 'part'
...
for(foottimespos in 1:(length(foottimes)-1))
   { #loop over time intervals
    selsub         <- (part[,"btime"]>(foottimes[foottimespos]))&(part[,"btime"]<=(foottimes[foottimespos+1]))
    subpart        <- part[selsub,]
    ...
    if(!is.null(fluxweighting))
       {
        ...
        if(fluxweighting=="CO2"|fluxweighting=="CO")
           {#for CO and CO2: time of day and day of week weighting
...
gridresulthr       <- gridresult[selsub,]
emissname          <- paste(gridresulthr[,"xpart"],gridresulthr[,"ypart"],gridresulthr[,"gridname"],sep="")
...
for (name in unique(emissname)){
	...
	if(!is.null(fluxweighting))emissgridwt<-getr(paste(veghead,fluxweight,".",name,sep=""),path=vegpath) #get CO emission grid
        sel        <- emissname==name
        gridname   <- unique(gridresulthr[sel,"gridname"])   #gridname can be 1~16, representing diff. resolutions of grid
        x          <- subpart[sel,"gitx"]
        y          <- subpart[sel,"gity"]
