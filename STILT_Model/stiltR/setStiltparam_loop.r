...
cat("setStiltparam_loop.r: starting...\n")

###### set directories ######
###### make sure they exist, at least the ones in the following first paragraph
#path<- "/home/dkivits/STILT/STILT_Model/Output/"
path<-"/scratch-shared/dkivits/STILT/footprints/" 		#path where output gets saved,
                                                         # also input data (Receptor locations and times)
                                                         # and boundary mixing ratio objects are read from 'path'
pathBinFootprint<-"/home/dkivits/STILT/STILT_Model/Output/PARIS/"
shlibpath <- "/home/dkivits/R/x86_64-pc-linux-gnu-library/3.4/"   # R extensions
metpath <- "/home/dkivits/arl_test/" # where met data are stored in ARL format
...
rundir <- "/home/dkivits/STILT/STILT_Model/STILT_Exe/"                               # specifies main directory where different directories are found to run hymodelc

cat("setStiltparam_loop.r: rundir=", rundir,"\n")
cat("setStiltparam_loop.r: metpath=", metpath,"\n")
...
nhrs <- -60            # for how many hours to run (time-backward runs negative).
nparstilt <- 250        # how many particles per receptor? 100 of for receptor oriented modeling with dynamic grid resolution is ok
...
###### define receptors ######
Timesname           <- paste('.RDataTimes_',stationfile$code[i],'.hf',sep='')  # name of object containing frac. julian, lat, lon, agl (m) as receptor information;
...
numpix.x             <- 250                   # number of pixels in x directions in grid (188 or 47)
numpix.y             <- 390                   # number of pixels in y directions in grid (108 or 27)
lon.res              <- (35 - -15)/(numpix.x -1) # resolution in degrees longitude
#lon.res              <- (35 - -15)/(numpix.x) # resolution in degrees longitude
lat.res              <- (72 - 33)/(numpix.y) # resolution in degrees latitude
