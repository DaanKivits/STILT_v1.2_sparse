...
#path<- "/home/awoude/STILT/STILT_Model/Output/"
path<-"/scratch/shared/awoude/footprints/" 		#path where output gets saved,
                                                         # also input data (Receptor locations and times)
                                                         # and boundary mixing ratio objects are read from 'path'
pathBinFootprint<-"/home/awoude/STILT/STILT_Model/Output/InGOS/FOOTPRINTS_stilt03/"
shlibpath <- "/home/awoude/R/x86_64-pc-linux-gnu-library/3.4/"   # R extensions
metpath             <- "/home/awoude/STILT/WRF-ARL-converter/metprog/grib2arl/arl/" # where met data are stored in ARL format
vegpath<-""#"/Net/Groups/BSY/data/WRF_STILT_prepro/VPRM/VPRM_input_STILT_EU2_2011/" 	#path to get to surface fluxes and vegetation grids at various resolutions; not needed for VPRM
rundir <- "/home/awoude/STILT/STILT_Model/STILT_Exe/"                               # specifies main directory where different directories are found to run hymodelc
...
###### basic parameters ######
nhrs <- -24               # for how many hours to run (time-backward runs negative).
nparstilt <- 500 # how many particles per receptor? 100 of for receptor oriented modeling with dynamic grid resolution is ok
...
metsource<-c("ecmw")   # Source of Meteorological data, for analysis runs (not forecasts)
...
varstrajec<-c("time","lat","lon","agl","zi","index","foot", "dmass")
...
overwrite=T             # T: rerun hymodelc, even if particle location object for same starting location and time is found;
...
#foottimes <- c(0,1) # vector of times (backtimes) in hours between which footprint is computed
#foottimes <- c(0,1,2,3,4,5,6) # Only the first timestep (one per day)
...
numpix.x             <- 250 #201 #251                   # number of pixels in x directions in grid (188 or 47)
numpix.y             <- 390 #157 #391                   # number of pixels in y directions in grid (108 or 27)
lon.res              <- (35.1 - -14.9)/(numpix.x -1) # resolution in degrees longitude
lat.res              <- (71.95 - 33.05)/(numpix.y -1) # resolution in degrees latitude
lon.ll               <- -15.0                 # lower left corner of grid
lat.ll               <- 33.00                   # lower left corner of grid
