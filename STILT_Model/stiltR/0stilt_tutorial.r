...
###### set directories ######
path<-"/home/awoude/"             #path where output gets saved, 
                                        #also input data (Receptor locations and times) 
                                        #and boundary mixing ratio objects are read from 'path'
metpath<-"/projects/0/ctdas/RINGO/Metfiles/"        #where met data are stored in ARL format
...
yr<-2016;mon<-1;day<-12;hr<-18
lat<- 50.53572;lon<-- 11.172;agl<-30

###### Run STILT ######
info<-Trajec(yr=yr,mon=mon,day=day,hr=hr,lat=lat,lon=lon,agl=agl,nhrs=nhrs,
               doublefiles=T,metd=c("wrf"),delt=delt,veght=veght,nummodel=nummodel,
...
numpix.x             <- 260                   # number of pixels in x directions in grid (188 or 47)
numpix.y             <- 240                   # number of pixels in y directions in grid (108 or 27)
lon.res              <- (19.950-  -5.950)/(numpix.x-1) # resolution in degrees longitude
lat.res              <- (54.975-  43.025)/(numpix.y-1) # resolution in degrees latitude
lon.ll               <- -5.95                 # lower left corner of grid
lat.ll               <- 43.025                # lower left corner of grid
